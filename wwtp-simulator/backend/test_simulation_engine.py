"""
Test file to compare the original BSM2_ol implementation with the JSON-based simulation engine.
This test validates that the simulation engine produces similar results to the reference BSM2 implementation.
"""
...
async def test_json_simulation_engine():
    """Test the JSON-based simulation engine."""
    logger.info("Starting JSON simulation engine test...")
    
    # Load configuration
    config = load_bsm2_test_config()
    ...
    try:
        ...
        logger.info("JSON simulation completed successfully")
        
        # Extract effluent results (should be from combiner-effluent-1)
        effluent_component = 'combiner-effluent-1'
        if effluent_component in results['components']:
            effluent_results = results['components'][effluent_component]
            final_effluent = effluent_results[-1] if effluent_results else None
            logger.info(f"JSON simulation final effluent: {final_effluent}")
            return results
        else:
            logger.warning("No effluent component found in results")
            logger.info(f"Available components: {list(results['components'].keys())}")
            return results
    
    except Exception as e:
        logger.error(f"JSON simulation failed: {e}")
        return None

def test_original_bsm2_ol():
    """Test the original BSM2_ol implementation."""
    logger.info("Starting original BSM2_ol test...")
    
    try:
        # Create BSM2OL instance with same parameters
        bsm2_ol = BSM2OL(endtime=1, timestep=15 / 60 / 24, tempmodel=False, activate=False)
        
        # Run simulation
        for idx, _ in enumerate(bsm2_ol.simtime):
            bsm2_ol.step(idx)
        
        logger.info("Original BSM2_ol simulation completed successfully")
        logger.info(f"Original BSM2_ol final effluent: {bsm2_ol.y_eff_all[-1, :]}")
        ...
        logger.error(f"Original BSM2_ol simulation failed: {e}")
        return None
...
def compare_results(json_results, original_results):
    ...
    if 'combiner-effluent-1' in json_results['components']:
        json_effluent = json_results['components']['combiner-effluent-1']
        if json_effluent:
            json_final = json_effluent[-1]
            original_final = original_results['effluent'][-1, :]
            
            logger.info(f"JSON final effluent shape: {np.array(json_final).shape}")
            logger.info(f"Original final effluent shape: {original_final.shape}")
            ...
            if isinstance(json_final, (list, np.ndarray)) and len(json_final) >= 21:
                ...
                if np.allclose(json_final, original_final, rtol=0.1, atol=1.0):
                    logger.info("✅ Results are reasonably close!")
                else:
        ...
        logger.warning("No effluent component found in JSON results")
        logger.info(f"Available components: {list(json_results['components'].keys())}")

async def main():
    """Main test function."""
    logger.info("Starting simulation engine comparison test")
    
    # Test original BSM2_ol
    original_results = test_original_bsm2_ol()
