...
# the PID controller adjusts the flow rate of a separate substance (kla). the separate substance then reacts to the measured substance and increases its concentration in the tank
# the actuator controls the flow rate of the substance into the tank
# the system is a simple first-order system that models the dynamics of the tank (the measured substance is consumed at a constant rate)

# the sensor has a transfer function that models its response time and a noise model. It also has a minimum and maximum value
# the data flow is: input signal -> transfer function -> noise -> limit check -> output signal
# the PID controller has a proportional, integral, and derivative term, as well as an anti-windup mechanism. It also has a minimum and maximum value
# the data flow is: error signal -> proportional term -> integral term -> derivative term -> anti-windup term -> control signal
