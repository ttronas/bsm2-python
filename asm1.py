...
class ASM1reactor:
    ...
    def derivatives(self, t, y, y_in, parasm):
        ...
        proc2 = mu_H*(S_S_temp/(K_S+S_S_temp))*(K_OH/(K_OH+S_O_temp))*(S_NO_temp/(K_NO+S_NO_temp))*ny_g*X_BH_temp
        ...
        if self.kla < 0.0:
            ...
            dS_O = 1.0 / self.volume * (Q_in * (S_O_in - S_O)) + reac8 + KLa_temp * (SO_sat_temp - S_O)
    ...
    def output(self, timestep, step, y_in, parasm):
        ...
        # sol = solve_ivp(self.derivatives, t_span, self.y0, args=(y_in, parasm,), rtol=1e-13, atol=1e-13)
