...
def test_bsm2_ol():
    ...
    storageinit.ystinit = np.array(initdata[8]).astype(np.float64)
    ...
    timestep = data_in[1, 0] - data_in[0, 0]
    endtime = data_in[-1, 0]
    simtime = np.arange(0, endtime + timestep, timestep)
    ...
    for i, step in enumerate(tqdm(simtime)):
        ...
        yp_in_c, y_in_bp = input_splitter.outputs(y_in[i, :], (0, 0), reginit.Qbypass)
