...
def test_storage_dyn():
    ...
    print('Sludge at t =', endtime, 'd: \n', yst_out)
    print('Volume at t =', endtime, 'd: \n', yst_vol)

    # Values from 50 days dynamic simulation in Matlab (storage_test_dyn.slx)
    yst_out_matlab = np.array([33.2152509553435, 41.8798580642530, 10544.6314538123, 20874.3609507103, 15318.3975401057, 380.110859861603, 1449.37232064426, 0.189234381018648, 0.778303638555896, 26.5243175258826, 4.21913273099171, 849.450689053129, 7.57679382207250, 36425.1548438506, 190.028282672942, 11.4654536074904, 4.97422833876973e-34, 0, 0, 0, 0])
    ...
    assert np.allclose(yst_out, yst_out_matlab, rtol=1e-5, atol=1e-5)
    assert np.allclose(yst_vol, yst_vol_matlab, rtol=2e-2)  # tolerance is higher for volume because of the integration method. If the step size is reduced the result gets more accurate!
