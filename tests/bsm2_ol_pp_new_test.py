...
def test_bsm2_ol_pp_new():
    ...
    performance = PlantPerformance(pp_init.PP_PAR)
    ...
    pumpingenergy_compare = performance.pumpingenergy(
        pe_flow_compare, pp_init.PP_PAR[10:16], bsm2_ol_pp.timestep[0], evaltime
    )
    ...
    oci = performance.oci(
        pumpingenergy, aerationenergy, mixingenergy, 0, tot_tss_mass_bsm2, carb_mass, heat_demand, ch4_prod
    )
    logger.info('OCI = %s\n', str(oci))
    ...
    assert np.allclose(eval_array, eval_par_matlab, rtol=3e-1)
