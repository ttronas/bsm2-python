...
def reduce_asm1(asm1_arr, reduce_to=("SI", "SS", "XI", "XS", "XBH", "SNH", "SND", "XND", "TSS", "Q", "TEMP")):
    ...
    out : np.ndarray
        reduced ASM1 array
    """
    asm1_components = ("SI", "SS", "XI", "XS", "XBH", "XBA", "XP", "SO", "SNO", "SNH", "SND", "XND", "SALK", "TSS", "Q", "TEMP", "SD1", "SD2", "SD3", "XD4", "XD5")
    # raise error if asm1_arr is not of shape (:,21)
    if len(asm1_arr.shape) == 1:
        is_1d = True
        asm1_arr = np.expand_dims(asm1_arr, axis=0)
    if asm1_arr.shape[1] != 21:
        raise ValueError("ASM1 array must have 21 columns")
...
def expand_asm1(red_arr, red_components=("SI", "SS", "XI", "XS", "XBH", "SNH", "SND", "XND", "TSS", "Q", "TEMP"), expand_by={"XBA": 0, "XP": 0, "SO": 0, "SNO": 0, "SALK": 7, "SD1": 0, "SD2": 0, "SD3": 0, "XD4": 0, "XD5": 0}):
    """
    ...
    expand_by : Dict(str:int)
        components to be added to the reduced array. Defaults to all non-changing components in BSM2 influent file and their default values:
    ...
    if red_arr.shape[1] != len(red_components):
        raise ValueError("Reduced ASM1 array must have the same number of columns as red_components")
    ...
    for idx, component in enumerate(asm1_components):
        ...
        elif component in expand_by.keys():
            out[:,idx] = expand_by[component]
        else:
            warnings.warn("Component {} is not in red_components or expand_by. Component {} is set to 0".format(component))
            out[:,idx] = 0
