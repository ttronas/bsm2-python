...
class Oxygensensor:
    ...
    def measureSO(self, SO: np.ndarray, step: float, controlnumber: int, noise_SO: float, transferfunction: float, control: float) -> float:
        ...
        if step == 0:
            ...
            SO_slice = SO[((int(transferfunction/control)+1) - controlnumber):(int(transferfunction/control)+1)]
            tout_SO, yout_SO, xout_SO = signal.lsim((num_SO, den_SO), SO_slice, t_SO_lower15[0:controlnumber])
...
class PIaeration:
    def __init__(self, KLa_min: float, KLa_max: float, KSO: float, TiSO: float, TtSO: float, SOref: float, KLaoffset: float, SOintstate: float, SOawstate: float, kla_lim: float, kla_calc: float, use_antiwindup: bool = True):
        ...
        use_antiwindup : bool
            If True, antiwindup is used in the PI control. Default is True
        """
...
class KLaactuator:
    ...
    def real_actuator(self, kla: np.ndarray, step: float, controlnumber: int, transferfunction: float, control: float) -> float:
