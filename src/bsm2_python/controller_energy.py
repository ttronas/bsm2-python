...
from numba import float64, boolean, int32, int64
...
class ControllerEnergy:
    ...
    def schedule_production(self, time_diff: float, chps: list[CHP], boilers: list[Boiler],
                            biogas_storage: BiogasStorage, cooler: Cooler, flare: Flare, heat_net: HeatNet,
                            heat_demand: float, gas_production: float):
        """
        ...
        biogas_consumption_chps = 0 #np.sum([chp.max_gas_power_uptake * chp.Load for chp in chps]) / self.biogas.h_u
        for chp in chps:
            biogas_consumption_chps += chp.max_gas_power_uptake * chp.Load / self.biogas.h_u
        biogas_storage_after_chps = self.preditct_biogas_storage_vol(biogas_storage.vol, gas_production,
                                                                     biogas_consumption_chps, time_diff)
        ...
        biogas_consumption_boiler = 0
        for boiler in boilers:
            biogas_consumption_boiler += boiler.max_gas_power_uptake * boiler.Load / self.biogas.h_u
        biogas_storage_after_boilers = self.preditct_biogas_storage_vol(biogas_storage_after_chps, 0,
                                                                        biogas_consumption_boiler, time_diff)
    ...
    def calculate_load_chps(self, chps: list[CHP], biogas_storage: BiogasStorage):
        ...
        for i, chp in enumerate(chps):
            ...
            if not np.any(self.gas_storage_above_threshold[i, :]):
                ...
                pass
            if chp.UnderMaintenance:
                chp.Load = 0

    def calculate_temperature_after_chps(self, heat_net: HeatNet, chps: list[CHP], heat_demand: float):
        """
        Calculate the new temperature of the heat net after supplying heat for the fermenter and using heat from chps, methanation and electrolyzer.
        Arguments:
            chps: List, Numba-List of CHP objects
            heat_demand: float, fermenter heat demand of current timestep, kW
        Returns:
            float, temperature of heat net after using heat from chps, methanation and electrolyzer, °C
        """
        chps_heat = 0
        for chp in chps:
    ...
    def preditct_biogas_storage_vol(self, fill_level, inflow, outflow, time_diff: float):
        """
    ...
    def calculate_load_boilers(self, temperature_deficit: float, heat_net: HeatNet, boilers: list[Boiler],
                               biogas_storage_fill_level: float):
        """
        Calculate the load of the boilers.
        Arguments:
            temperature_deficit: float, temperature deficit of the heat net after supplying heat for the fermenter and using heat from chps, K
            heat_net: HeatNet, heat network object
            boilers: List, Numba-List of Boiler objects
            biogas_storage_fill_level: float, fill level of biogas storage after supplying biogas for the chps and using receiving biogas from the fermenter, Nm^3
        """
        ...
        for boiler in boilers:
            ...
            if load <= max_load_possible:
                ...
                if max_load_possible >= boiler.minimum_load:
    ...
    def calculate_load_cooler(self, temperature_surplus: float, heat_net: HeatNet, cooler: Cooler):
        """
        Calculate the load of the cooler.
        Arguments:
            temperature_surplus: float, temperature surplus of the heat net after supplying heat for the fermenter and using heat from chps, K
            heat_net: HeatNet, heat network object
    ...
    def calculate_load_flare(self, biogas_storeage_max_vol: float, biogas_storage_vol_prog: float, flare: Flare):
        """
