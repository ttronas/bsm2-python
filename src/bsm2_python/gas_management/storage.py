...
class BiogasStorage:
    def __init__(self, max_vol: float, p_store: float, vol_init: float, capex_sp: float, opex_factor: float,
                 biogas: GasMix, gas_composition: np.ndarray):
        """
        A class that represents a gas storage.
        Arguments:
            max_vol: maximum volume of the storage [Nm³]
            p_store: pressure of the storage [bar]
            vol_init: initial volume of the storage [Nm³]
            capex_sp: specific capital expenditure of the storage [€/Nm³]
            ...
            gas_composition: composition of the gas in the storage
        """
        self.tendency = 0
    ...
    def update_inflow(self, gas_flow_in: float, gas_flow_in_composition: np.ndarray, time_diff: float):
        """
    ...
    def update_outflow(self, gas_flow_out: float, time_diff: float):
        """
