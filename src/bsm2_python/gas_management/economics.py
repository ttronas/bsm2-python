...
PAYBACK_TIME = 20
ANNUITY = (BANK_INTEREST_RATE * ((1 + BANK_INTEREST_RATE) ** PAYBACK_TIME) /
           (((1 + BANK_INTEREST_RATE) ** PAYBACK_TIME) - 1))
...
class Economics:
    ...
    def calculate_debt_payment_timestep(self, time_diff: float, investment: float):
    ...
    def get_debt_payment(self, time_diff: float):  # additional to existing wwtp -> electrolyzer, methanation, storages
        ...
        payment_boilers = np.sum([self.calculate_debt_payment_timestep(time_diff, boiler.capex) for boiler in self.boilers])
        ...
        investment_total = np.sum([chp.capex for chp in self.chps]) \
                           + np.sum([boiler.capex for boiler in self.boilers]) + \
                           self.biogas_storage.capex + self.biogas_compressor.capex + self.flare.capex \
                           + self.cooler.capex
        payment_planning = self.calculate_debt_payment_timestep(time_diff, planning_permit_certificate * investment_total)
        payment_reserve = self.calculate_debt_payment_timestep(time_diff, reserve * investment_total)

        return payment_chps + payment_boilers + payment_biogas_storage + payment_biogas_compressor + payment_flare + \
               payment_cooler + payment_planning + payment_reserve
    ...
    def get_maintenance_costs(self, time_diff: float):
        ...
        maintenance_boilers = np.sum([boiler.capex * MAINTENANCE_COST / (HOURS_IN_YEAR / time_diff) for boiler in self.boilers])
        ...
        total_maintenance = maintenance_chps + maintenance_boilers + maintenance_biogas_storage \
                            + maintenance_biogas_compressor + maintenance_flare + maintenance_cooler
    ...
    def get_insurance_costs(self, time_diff: float):
        ...
        insurance_boilers = np.sum([boiler.capex * INSURANCE_COST / (HOURS_IN_YEAR / time_diff) for boiler in self.boilers])
        ...
        total_insurance = insurance_chps + insurance_boilers + insurance_biogas_storage \
                          + insurance_biogas_compressor + insurance_flare + insurance_cooler

        return total_insurance

    def get_staff_cost(self, time_diff: float):
        return 0
    ...
    def get_total_opex(self, time_diff: float):
        return self.get_maintenance_costs(time_diff) + self.get_insurance_costs(time_diff) + \
               self.get_staff_cost(time_diff)
