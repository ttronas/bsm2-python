...
class Evaluation:
    def __init__(self, total_sim_steps: int, chps_count: int, boilers_count: int, chp_report_shape: int,
                 boiler_report_shape: int, flare_report_shape: int, cooler_report_shape: int):
        """
    ...
    def calculate_data(self, step: int, fermenter: Fermenter, chps: list[CHP], boilers: list[Boiler], flare: Flare,
                       cooler: Cooler, biogas_storage: BiogasStorage, heat_net: HeatNet, incomes: float,
                       expenditures: float):
        """
        ...
        self.biogas_storage_all[step, :] = np.array([biogas_storage.vol, biogas_storage.tendency, biogas_storage.deficiency, biogas_storage.surplus])
        self.heatnet_all[step] = heat_net.temperature
        self.fermenter_all[step] = np.array([fermenter.gas_production, fermenter.heat_demand, fermenter.electricity_demand])
        electricity_demand_wwtp = fermenter.electricity_demand - np.sum([chp.Products[chp_init.ELECTRICITY] for chp in chps])
        ...
        if step == 0:
            ...
            self.economics_all[step] = np.array([incomes, expenditures, self.economics_all[step - 1, 2] + incomes - expenditures])
    ...
    def display_data(self):
        ...
        for i in range(self.chps_count):
            chp_data = pd.DataFrame(self.chp_stat_all[:, i, :], columns=['Load', 'Maintenance', 'Electricity', 'Heat', 'Biogas'])
        ...
        biogas_storage_data = pd.DataFrame(self.biogas_storage_all, columns=['Level', 'Tendency', 'Deficiency', 'Surplus'])
        heatnet_data = pd.DataFrame(self.heatnet_all, columns=['Temperature'])
        fermenter_data = pd.DataFrame(self.fermenter_all, columns=['Gas Production', 'Heat Demand', 'Electricity Demand'])
        net_electricity_data = pd.DataFrame(self.net_electricity_all, columns=['Electricity Net to WWTP', 'Electricity WWTP tp Net'])
        ...
        axes = heatnet_data.plot(subplots=True, figsize=(10, 10), title=f'Heatnet')
        axes[0].set_ylabel('°C')

        axes = biogas_storage_data.plot(subplots=True, figsize=(10, 10), title=f'Biogas Storage')
        axes[0].set_ylabel('Nm³')
        axes[1].set_ylabel('Nm³/h')
        axes[2].set_ylabel('Nm³/h')
        axes[3].set_ylabel('Nm³/h')

        axes = fermenter_data.plot(subplots=True, figsize=(10, 10), title=f'Fermenter')
        axes[0].set_ylabel('Nm³/h')
        axes[1].set_ylabel('kW')
        axes[2].set_ylabel('kW')

        axes = net_electricity_data.plot(subplots=True, figsize=(10,10), title=f'Net Electricity')
        ...
        axes = flare_data.plot(subplots=True, figsize=(10,10), title=f'Flare')
        axes[0].set_ylabel('-')
        axes[1].set_ylabel('Nm³/h')

        axes = cooler_data.plot(subplots=True, figsize=(10,10), title=f'Cooler')
        axes[0].set_ylabel('-')
        axes[1].set_ylabel('kW')

        axes = economics_data.plot(subplots=True, figsize=(10,10), title=f'Economics')
        axes[0].set_ylabel('€')
        axes[1].set_ylabel('€')
        axes[2].set_ylabel('€')
    ...
    def save_data(self, data_path_base: str):
        ...
        output = pd.concat([pd.DataFrame(sep_arr, columns=['Net Electricity']), net_electricity_data,
            # pd.DataFrame(sep_arr, columns=['Electricity Price']), elec_price_data,
            pd.DataFrame(sep_arr, columns=['Biogas Storage']), biogas_storage_data,
            pd.DataFrame(sep_arr, columns=['Flare']), flare_data,
            pd.DataFrame(sep_arr, columns=['Heatnet']), heatnet_data,
        ], axis=1)
        ...
        for i in range(self.boilers_count):
            output = pd.concat([output, pd.DataFrame(sep_arr, columns=['Boiler ' + str(i+1)]), boilers_data[i]], axis=1)

        # output = pd.concat([output, pd.DataFrame(sep_arr, columns=['Incomes', 'Expenditures', 'Cumulative Cash Flow']), economics_data], axis=1)
