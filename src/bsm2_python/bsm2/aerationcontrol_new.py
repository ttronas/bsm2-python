...
class PID:
    ...
    def output(self, signal, dt):
        dt = np.array([0, dt])
        ode_integral = odeint(function_int, self.integral, dt,
                              args=(self.setpoint, signal, self.k, self.t_i), tfirst=True)
        ...
        antiwindup_integral = odeint(function_aw, self.aw_init, dt,
                                     args=(self.prev_lim, self.prev_signal, self.t_t), tfirst=True)
